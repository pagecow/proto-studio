...
## Proto Studio v1.2.0 — collapsible panels + a CSS Grid trap worth remembering

**Feature:** each side panel (projects / Proto Agent) collapses to a 36px rail that stays visible, so the way back is always one click — per-panel « chevrons, a **Full canvas** toolbar button toggling both, and ⌘\\ (with ⇧⌘\\ for the agent alone). State persists in `store.collapsed`.

🔴 **THE TRAP (cost a debugging cycle): never `display:none` a CSS Grid child to "hide" it.** `AppRuntimeView`… sorry — in `.pf-shell` the three columns are auto-placed across 5 tracks (`panel, grip, panel, grip, 1fr`). Hiding the grip `<div>`s with the `hidden` attribute removed them **from auto-placement entirely**, so every later item shifted one track left: the agent column landed in the 7px grip track and the canvas in the 36px rail track, leaving the 1114px track EMPTY. The canvas *shrank* from 451px to 52px when "collapsed". Fix: make the grip tracks CSS variables (`--pf-grip-left/--pf-grip-mid`) and set them to `0px` — the element stays in flow, the track just goes to zero. Add `min-width:0` + `pointer-events:none` for safety. (Generic rule: a hidden grid item is removed from placement; use a 0-size track, explicit `grid-column`, or `visibility:hidden` if you must keep placement.)

**Verification numbers (measured in-app, not assumed):** canvas 451px expanded → 582 (projects away) → 997 (agent away) → **1128 (both away)**; every rail reappears only while its own panel is collapsed, and both the rails and the button restore to exactly 451. Probe drove the real UI entry points (`.click()` on the rails and the button), not just the internal functions.

**Probe pattern that keeps paying off:** a temporary `probe()` in `boot()` writing to `drive.writeFile('_probe/<name>.json')`, read from `~/Library/Application Support/com.chatoss.desktop/drive/<appid>/_probe/<name>.json` — then REMOVE the probe before shipping. Measure `getComputedStyle(shell).gridTemplateColumns` + each child's `getBoundingClientRect()` when a layout looks wrong; the resolved track string plus per-child left/width pinpointed this instantly.

**Release chain (repeatable):** bump `app.json` version → update README + `tmp/release/notes-X.Y.Z.md` → commit (`-m` per paragraph, `Built with ChatOSS.ai` last) → push → `zip -q -r tmp/release/app-vX.Y.Z.aip app.json index.html style.css app.js icon.svg` → `gh release create vX.Y.Z --target main --title … --notes-file …` → upload the `.aip` + byte-identical `.zip` → verify with `gh api …/releases/latest`. Shipping now: **v1.2.0** (assets 25,365 bytes each). The Store listing still points at the repo's latest release, so installs pick up 1.2.0 with no re-review; the *listing description* update remains unsubmitted per the user's choice.
